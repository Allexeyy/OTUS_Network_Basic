...
h.	Изучите свойства по умолчанию интерфейса FastEthernet, который используется компьютером PC1.
    ...
    Какой MAC-адрес у интерфейса? - 2я комманда не показала что f0/1 есть mac-адрес (но его можно позже назначить)
...
с. Проводим настройку консоли: ограничиваем доступ паролем и изменяем параметр чтобы консольные сообщения не ппрерывали комманд:
